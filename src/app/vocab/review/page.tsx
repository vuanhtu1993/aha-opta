...
import type { QuizQuestion } from "@/app/api/vocab/review-session/route";
...
function VocabReviewContent() {
  ...
  const fetchSession = async () => {
    try {
      setLoading(true);
      setError(null);
      const url = practiceAll
        ? "/api/vocab/review-session?limit=15&practice_all=true"
        : "/api/vocab/review-session?limit=15";
      const res = await fetch(url);
      const data = await res.json();

      if (!res.ok) {
        throw new Error(data.error || "Không thể tải phiên ôn tập.");
      }
  ...
  // When moving to new question, reset timer
  useEffect(() => {
  ...
  const formatIntervalDays = (nextDueStr?: string) => {
    ...
    if (diffHours < 24) return `${Math.max(1, diffHours)} giờ`;
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-6 text-center space-y-4">
        <div className="w-12 h-12 rounded-2xl bg-amber-50 dark:bg-amber-950/40 text-amber-500 flex items-center justify-center animate-spin">
          <Sparkles className="w-6 h-6" />
        </div>
        <p className="text-xs font-bold text-slate-500 dark:text-slate-400">
          Đang chuẩn bị phiên trắc nghiệm SRS...
        </p>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-6 text-center space-y-4">
        ...
        <button
          ...
          className="px-4 py-2 bg-amber-400 text-slate-900 font-bold text-xs rounded-xl"
        >
              ...
              <div className="flex items-center gap-2">
                <span
                  className={`px-2 py-0.5 rounded-md text-[10px] font-extrabold border ${
                    getRatingBadge(currentResult.rating).color
                  }`}
                >
