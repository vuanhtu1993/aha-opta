...
export async function dataFetcherNode(state: OptaStateType): Promise<Partial<OptaStateType>> {
  ...
  const homeTeam = await Team.findById(state.homeTeamId).select("name shortName country group confederation fifaRanking eloRating eloRank eloLastSynced recentEloMatches stats").lean();
  const awayTeam = await Team.findById(state.awayTeamId).select("name shortName country group confederation fifaRanking eloRating eloRank eloLastSynced recentEloMatches stats").lean();
